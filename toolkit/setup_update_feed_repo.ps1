...
<#
.SYNOPSIS
  Collega PublishTarget + FeedBaseUrl alla repo GitHub PUBBLICA esistente MMX-NET.
#>
...
if (-not $exists) {
    throw "Repo $user/$repoName non trovata. Creala su GitHub (pubblica) o correggi il nome — non inventare una repo nuova da questo script."
}
...
Feed pubblico **MMX-Net** (``ac_update_manifest.json``, ``ac_version.json``, ``ac-update.zip``).
...
Write-Host "  Prossimi passi: repo PUBBLICA: amici senza PAT (solo updateFeedUrl); write solo owner; dopo CARICA: push_update_feed.ps1"
